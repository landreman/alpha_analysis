...
- Milestone 8's `CriticalCurves` stores point and segment classifications plus ordered, cumulative-arc-length polylines using global vertex IDs. Periodic stitching is restricted to uniquely matched lower/upper degree-one endpoints carrying `PERIODIC_SEAM` provenance; `GAMMA_MAX` polylines are ready for Milestone 9's common-parameter transition mapping, while `DEGENERATE` or `UNRESOLVED` results must remain explicit rather than being mapped as generic transitions.
- Milestone 8 refines standalone critical-curve arrays. Inserting those vertices into `SurfaceMesh` and clearing Milestone 7's boundary `refinement_blocked` edges is deferred to Milestone 10's constrained cuts; local `b-B_extremum` gap scaling and transition-sampling convergence belong to Milestone 9. The combined pitch-surface diagnostic must add `EDGE`/`AXIS` overlays when Milestone 15 assembles the pipeline view.
