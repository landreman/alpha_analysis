...
- Five-equilibrium validation through Milestone 7 showed that projected midpoint failures and face-inverting bisections must also remain explicit `refinement_blocked` candidates; their separate `projection_failed` and `face_validity_failed` diagnostics prevent a different level-set root or an inverted child from being accepted. The same sweep established that long regular wells may require extrema-delimited quadrature with a summed global error budget, and that `max_field_periods` remains a convergence control: the nfp=4 low-resolution case at radially global `lambda_n=0.95` converges at 180 field periods with a cap of 256 (and agrees at 512), while the default cap of 128 correctly remains `MAX_PERIODS`.
